  ...
  if (NULL != pDevLabel) {
    ...
    if (NULL != pRaw) {
      ...
      if ((rvalue >= 0) && (NULL != pRaw->pSocket)) {
        ...
        strncpy(pRaw->hdr.str, MERC_NAME, 4);

        /* Set the protocol type */
        pRaw->uProtocol = MERC_PROTOCOL;

        /* Set the protcol version */
        pRaw->uVersion = MERC_INT_VERSION;
        ...
        if (NULL != pDev) {
          ...
          MERC_MSG("Didn't find network device, we will broadcast it");
...
static int privateRecvRawThread(void *pPtr)
{
  ...
  while ((false == kthread_should_stop()) &&
    ...
    if (true == pRaw->bConnected) {
      ...
      if (NULL != pSkb) {
        pHdr = (MERC_RAW_HEADER *)pSkb->data;
	...
	if (pSkb->len >= uHdrLen) {
	  ...
	  if (pRaw->uProtocol == ntohs(pHdr->uProtocol)) {
		...
		/* call the merc80211 side, to recv packet. */
		//merc80211Recv(pRaw->pData, pSkb);
...
void *mercNetConnectRaw(void *pData, char *pDevLabel)
{
  ...
  if (NULL != pRaw) {
    sprintf(pRaw->pRecvName, "merc-%s-recv", pDevLabel);
    pRaw->pData = pData;
			     ...
			     char *pHdr, unsigned int uHdrSize)
{
  ...
  if (NULL != pRaw) {
    if (true == pRaw->bConnected) {
      ...
      /* Point to the ethernet header + merc datagram stuff */
      sioVec [uvloc].iov_base = (char *)&khdr;
      sioVec [uvloc].iov_len = sizeof(MERC_RAW_HEADER);
